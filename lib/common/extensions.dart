...
extension ByteToTextSize on int {
  ...
  String toByteSize() {
    if (byByteSize < 1024) {
      return "mold:extension:byte %1s".translate(args: ["$byByteSize"]);
    } else if (byKByteSize < 1024) {
      return "mold:extension:kilobyte %1s".translate(args: ["$byKByteSize"]);
    } else if (byMByteSize < 1024) {
      return "mold:extension:megabyte %1s".translate(args: ["$byMByteSize"]);
    }
    return "mold:extension:gigabyte %1s".translate(args: ["$byGByteSize"]);
  }
}
...
extension Iterables<E> on Iterable<E> {
  Map<K, List<E>> groupBy<K>(K Function(E) keyFunction) => fold(
        <K, List<E>>{},
        (Map<K, List<E>> map, E e) => map..putIfAbsent(keyFunction(e), () => <E>[]).add(e),
      );
}
...
extension ListExtensions<T> on Iterable<T> {
  ...
  Iterable<T> whereNotNull(bool test(T element)) => this.where((e) => test(e));

  Iterable<T> whereNot(bool test(T element)) => this.where((e) => !test(e));

  T? firstWhere(bool test(T element)) => this.firstWhere(test, orElse: () => null as T);

  T? lastWhere(bool test(T element)) => this.lastWhere(test, orElse: () => null as T);
}
...
extension ListNullableExtensions<T> on Iterable<T?> {
  ...
  Iterable<T?> whereNot(bool test(T element)) => this.where((e) => !test(e as T));
}
